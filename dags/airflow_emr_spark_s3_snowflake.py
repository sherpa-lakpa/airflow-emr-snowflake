...
def create_emr_cluster():
  cluster_id = client.run_job_flow(
    ...
    LogUri="s3://emr-airflow-snowflake/emr_logs/",
...
with dag:
  ...
  ingest_layer = PythonOperator(
    ...
    op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','s3://us-east-1.elasticmapreduce/libs/script-runner/script-runner.jar',[ 's3://emr-airflow-snowflake/scripts/ingest.sh']],
    dag=dag, 
  )
  ...
  transform_layer = PythonOperator(
    ...
    op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','command-runner.jar',[ 'spark-submit',
              '--master', 'yarn',
              '--deploy-mode', 'cluster',
              's3://emr-airflow-snowflake/scripts/transform.py']],
    dag=dag, 
  )
