...
dag = DAG(
    dag_id="online_retail_transaction", default_args=args, schedule_interval=None
)
...
def create_emr_cluster():
  cluster_id = client.run_job_flow(
    ...
    Instances={
      ...
      'Ec2SubnetId': 'subnet-008682e3a7ba87b74',
    },
    LogUri="s3://online-retail-transaction/emr_logs/",
...
with dag:
  ...
  ingest_layer = PythonOperator(
    ...
    op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','s3://us-east-1.elasticmapreduce/libs/script-runner/script-runner.jar',[ 's3://online-retail-transaction/scripts/ingest.sh']],
    dag=dag, 
  )
  ...
  transform_layer = PythonOperator(
    ...
    op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','command-runner.jar',[ 'spark-submit',
              '--master', 'yarn',
              '--deploy-mode', 'cluster',
              's3://online-retail-transaction/scripts/transform.py']],
    dag=dag, 
  )
